...
# 这个装饰器表明这个函数是一个模版标签
# takes_context = True 表示接收上下文对象，就是前面所说的封装了各种变量的 Context 对象。
def paginate(context, object_list, page_count):
    # context为Context对象，object_list为分页的对象，page_count表示每页的数量

    left = 3     # 当前页码左边显示几个页码号-1（减一），比如3就显示2个
    ...
    try:
        ...
        # 调用了两个辅助函数，根据当前页得到左右的页码号
        # 比如设置成获取左右两边2个页码号，那么假如当前页是5，则 pages = [3,4,5,6,7],
        # 当然一些细节需要处理，比如如果当前页是2，那么获取的是pages = [1,2,3,4]

    except PageNotAnInteger:
        # 异常处理，如果用户传递page值的不是整数，则把第一页的值返回给他
        object_list = paginator.page(1)
        context['current_page'] = 1     # 当前页是1
        pages = get_right(context['current_page'], right, paginator.num_pages)

    except EmptyPage:
        # 如果用户传递的page值是一个空值，那么把最后一页的值返回给他
        object_list = paginator.page(paginator.num_pages)
        context['current_page'] = paginator.num_pages  # 当前是最后一页，num_pages的值是总分页数
        pages = get_left(context['current_page'], left, paginator.num_pages)

    context['article_list'] = object_list       # 把获取到的分页数据封装到上下文中
    context['pages'] = pages                    # 把页码号列表封装进去
    context['first_page'] = 1                   # 第一页页码号为1
    context['last_page'] = paginator.num_pages  # 最后一页的页码号

    try:
        """
        获取pages列表第一个值和最后一个值，主要用于在是否该插入省略号的判断，
        ...
        # +1的原因是为了方便判断，在模版文件中将会体会到其作用

    except IndexError:
        context['pages_first'] = 1   # 发生异常说明只有1页
        context['pages_last'] = 2    # 1+1后的值

    return ''                        # 如果不加‘’，首页会显示None
...
def get_left(current_page, left, num_pages):
    ...
    if current_page == 1:
        ...
        l = [i - 1 for i in xrange(current_page, current_page - left, -1) if i - 1 > 1]
        l.sort()
        return l
    l = [i for i in xrange(current_page, current_page - left, -1) if i > 1]
    l.sort()
    return l
